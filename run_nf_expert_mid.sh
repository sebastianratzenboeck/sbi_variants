...
#SBATCH -o nf_expert_mid_%j.out
#SBATCH -e nf_expert_mid_%j.err
...
OUTPUT_DIR="/n/holystore01/LABS/itc_lab/Lab/to-Sebastian/age_mixture_nf_experts/mid"
RUN_NAME="nf_expert_mid_tau-0_epochSize5M_maxEpochs300"
...
python -u train_variant_sbi_nf_theta.py \
  ...
  --run-name "${RUN_NAME}" \
  --train-indices "${SPLIT_DIR}/train_indices_mid.npy" \
  --val-indices "${SPLIT_DIR}/val_indices_mid.npy" \
  --epochs 300 \
