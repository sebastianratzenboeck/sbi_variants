...
#SBATCH -o nf_expert_old_%j.out
#SBATCH -e nf_expert_old_%j.err
...
OUTPUT_DIR="/n/holystore01/LABS/itc_lab/Lab/to-Sebastian/age_mixture_nf_experts/old"
RUN_NAME="nf_expert_old_tau-0_epochSize5M_maxEpochs300"
...
python -u train_variant_sbi_nf_theta.py \
  ...
  --run-name "${RUN_NAME}" \
  --train-indices "${SPLIT_DIR}/train_indices_old.npy" \
  --val-indices "${SPLIT_DIR}/val_indices_old.npy" \
  --epochs 300 \
